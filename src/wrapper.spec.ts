...
describe("Cycles", () => {
  it("detects trivial cycles", () => {
    const c: Signal.Computed<any> = new Signal.Computed(() => c.get());
    expect(() => c.get()).toThrow();
  });
  it("detects slightly larger cycles", () => {
    const c: Signal.Computed<any> = new Signal.Computed(() => c2.get());
    const c2: Signal.Computed<any> = new Signal.Computed(() => c.get());
    const c3: Signal.Computed<any> = new Signal.Computed(() => c2.get());
...
describe("Custom equality", () => {
  ...
  it("does not leak tracking information", () => {
    ...
    const cutoff = vi.fn(
      (a: number, b: number) => Math.abs(a - b) < epsilon.get()
    );
...
describe("Receivers", () => {
  it("is this for computed", () => {
    let receiver: Signal.Computed<any> | undefined = undefined;
    // Add type annotation to 'this'
    const c = new Signal.Computed(function (this: Signal.Computed<any>) {
      receiver = this;
    });
...
describe("watch and unwatch", () => {
  ...
  it("understands dynamic dependency sets", () => {
    ...
    const c = new Signal.Computed(() => (d++, which.get()));
    const w = new Signal.subtle.Watcher(() => n++);
